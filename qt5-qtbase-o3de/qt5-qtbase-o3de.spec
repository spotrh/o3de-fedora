...
%global oldname qt5-qtbase

Name:    %{oldname}-o3de
Summary: Qt5 - QtBase components (o3de fork)
Version: 5.15.2
Release: 15%{?dist}
...
## o3de patches
Patch300: 0001-Add-native-frame-around-QDockWidgetGroupWindow.patch
Patch301: 0002-Allow-to-prepend-instead-of-append-when-adding-dock-.patch
Patch302: 0003-Fix-QEvent-globalPos-being-different-than-QCursor-po.patch
Patch303: 0004-Delay-hidding-the-roll-effect-by-1-frame-to-avoid-fl.patch
Patch304: 0005-Support-QSslSocketBackendPrivate-verify-on-macOS.patch
Patch305: 0006-Don-t-recalculate-stylesheet-rules-when-reparenting.patch
Patch306: 0007-Initial-implementation-of-QSslCertificate-toText-for.patch
Patch307: 0008-Break-infinite-recursion-when-processing-key-events.patch
Patch308: 0009-Add-a-QPalette-color-role-for-placeholder-texts.patch
Patch309: 0010-Introduce-a-style-hint-to-control-text-tooltips-wrap.patch
Patch310: 0011-Make-the-repolish-minimization-optimization-opt-in.patch
Patch311: 0012-Allow-widgets-to-propagate-their-style-to-children.patch
Patch312: 0013-Decouple-QStyleSheetStyle-from-qApp-styleSheet.patch
Patch313: 0014-Introduce-Qt-AA_ManualStyleSheetStyle.patch
Patch314: 0015-Save-dialog-Don-t-add-the-default-suffix-if-suffix-s.patch
Patch315: 0016-Fix-Application-flags-they-can-t-be-bigger-than-31.patch
Patch316: 0017-Make-sure-unsetting-a-stylesheet-will-restore-change.patch
Patch317: 0018-QStyleSheetStyle-repolish-run-on-all-descendants.patch
Patch318: 0019-Make-QTreeView-setAutoExpandDelay-only-work-for-expa.patch
Patch319: 0020-QPA-update-cached-frame-margins-on-reparenting.patch
Patch320: 0021-Add-style-option-to-adjust-QMenu-placement.patch
Patch321: 0022-Platform-specific-arguments-parse-a-comma-separated-.patch
Patch322: 0023-Align-menu-text-if-one-of-the-items-is-checkable-ada.patch
Patch323: 0024-Fix-sub-menu-alignment.patch
Patch324: 0025-Add-high-DPI-scale-factor-rounding-policy-C-API.patch
Patch325: 0026-QHighDPI-Fix-broken-scaling-of-QPoint-F.patch
Patch326: 0027-Add-AZ_QT_VERSION-to-qnamespace.h-to-track-internal-.patch
Patch328: 0029-Fix-icon-offset.patch
Patch329: 0030-Cache-latin1-to-UTF8-className-conversion-lookups-in.patch
Patch330: 0031-Remove-AA_ImmediateWidgetCreation.patch
Patch331: 0032-Add-support-for-up-to-64-application-attributes.patch
...
%endif
Requires: %{oldname}-common = %{version}-%{release}
...
Requires: %{oldname}-devel%{?_isa} = %{version}-%{release}
...
Requires: cups-devel
Requires: %{oldname}-private-devel%{?_isa} = %{version}-%{release}
...
# glxinfo
Requires: glx-utils
Requires: %{oldname}-gui%{?_isa} = %{version}-%{release}
...
%patch300 -p1 -b .add-native-frame
%patch301 -p1 -b .allow-to-prepend
%patch302 -p1 -b .fix-QEvent-globalPos
%patch303 -p1 -b .delay-hiding-the-roll-effect
%patch304 -p1 -b .support-QSslSocketBackendPrivate
%patch305 -p1 -b .dont-recalculate-stylesheet-rules
%patch306 -p1 -b .initial-implementation-of-QSslCertificate
%patch307 -p1 -b .break-infinite-recursion
%patch308 -p1 -b .add-a-QPalette-color-role
%patch309 -p1 -b .introduce-a-style-hint
%patch310 -p1 -b .make-the-repolish-minimization-opt-in
%patch311 -p1 -b .allow-widgets-to-propogate-their-style
%patch312 -p1 -b .decouple-QStyleSheetStyle-from-qApp
%patch313 -p1 -b .introduce-qt-AA_ManualStyleSheetStyle
%patch314 -p1 -b .save-dialog-dont-add-the-default-suffix
%patch315 -p1 -b .fix-application-flags
%patch316 -p1 -b .make-sure-unsetting-a-styleheet
%patch317 -p1 -b .QStyleSheetStyle-repolish-run-on-descendants
%patch318 -p1 -b .make-QTreeView-setAutoExpandDelay
%patch319 -p1 -b .qpa-update-cached-frame-margins
%patch320 -p1 -b .add-style-option-to-adjust-QMenu
%patch321 -p1 -b .platform-specific-arguments-parse
%patch322 -p1 -b .align-menu-text
%patch323 -p1 -b .fix-sub-menu-alignment
%patch324 -p1 -b .add-high-dpi-scale-factor-rounding
%patch325 -p1 -b .QHighDPI-fix-broken-scaling
%patch326 -p1 -b .add-AZ_QT_VERSION
%patch328 -p1 -b .fix-icon-offset
%patch329 -p1 -b .cache-latin1-to-utf8
%patch330 -p1 -b .remove-AA_ImmediateWidgetCreation
%patch331 -p1 -b .add-support-for-64-application-attributes
...
mv include/QtCore include/QtCoreo3de
mv include/QtNetwork include/QtNetworko3de
mv include/QtWidgets include/QtWidgetso3de
pushd include
ln -s QtCoreo3de QtCore
cd QtCore/%{version}/
mv QtCore QtCoreo3de
ln -s QtCoreo3de QtCore
cd ..
ln -s QtCore QtCoreo3de
ln -s QtCoreDepends QtCoreo3deDepends
cd ..
ln -s QtNetworko3de QtNetwork
cd QtNetwork/%{version}/
mv QtNetwork QtNetworko3de
ln -s QtNetworko3de QtNetwork
cd ..
ln -s QtNetwork QtNetworko3de
ln -s QtNetworkDepends QtNetworko3deDepends
cd ..
ln -s QtWidgetso3de QtWidgets
cd QtWidgets/%{version}/
mv QtWidgets QtWidgetso3de
ln -s QtWidgetso3de QtWidgets
cd ..
ln -s QtWidgets QtWidgetso3de
ln -s QtWidgetsDepends QtWidgetso3deDepends
cd ..
popd
...
# o3de ugliness
sed -i 's|= QtCore|= QtCoreo3de|g' src/corelib/corelib.pro
sed -i 's|= QtNetwork|= QtNetworko3de|g' src/network/network.pro
sed -i 's|= QtWidgets|= QtWidgetso3de|g' src/widgets/widgets.pro
%make_build
...
# multilib: qconfig.h
  mv %{buildroot}%{_qt5_headerdir}/QtCoreo3de/qconfig.h %{buildroot}%{_qt5_headerdir}/QtCoreo3de/qconfig-%{__isa_bits}.h
  install -p -m644 -D %{SOURCE5} %{buildroot}%{_qt5_headerdir}/QtCoreo3de/qconfig.h
%endif
...
# o3de: Strip it to the minimum
rm -rf %{buildroot}%{_sysconfdir}/X11/xinit
rm -rf %{buildroot}%{_qt5_libdir}/libQt5Gui.so* %{buildroot}%{_qt5_libdir}/libQt5OpenGL.so* %{buildroot}%{_qt5_libdir}/libQt5PrintSupport.so* %{buildroot}%{_qt5_libdir}/libQt5XcbQpa.so*
rm -rf %{buildroot}%{_qt5_plugindir}/
rm -rf %{buildroot}%{_qt5_libdir}/cmake/Qt5 %{buildroot}%{_qt5_libdir}/cmake/Qt5A* %{buildroot}%{_qt5_libdir}/cmake/Qt5Concurrent*
rm -rf %{buildroot}%{_qt5_libdir}/cmake/Qt5[D-M]* %{buildroot}%{_qt5_libdir}/cmake/Qt5[O-V]* %{buildroot}%{_qt5_libdir}/cmake/Qt5X*
%if 0%{?egl}
rm -rf %{buildroot}%{_qt5_libdir}/libQt5EglFSDeviceIntegration.so* %{buildroot}%{_qt5_libdir}/libQt5EglFsKmsSupport.so*
%endif
rm -rf %{buildroot}%{_qt5_libdir}/cmake/Qt5PrintSupport

%if 0%{?qtchooser}
rm -rf %{buildroot}%{_sysconfdir}/xdg/qtchooser
%endif
rm -rf %{buildroot}%{_sysconfdir}/xdg/QtProject/
rm -rf %{buildroot}%{_qt5_libdir}/libQt5Concurrent.so* %{buildroot}%{_qt5_libdir}/libQt5DBus.so* %{buildroot}%{_qt5_libdir}/libQt5Sql.so* %{buildroot}%{_qt5_libdir}/libQt5Test.so* %{buildroot}%{_qt5_libdir}/libQt5Xml.so*
rm -rf %{buildroot}%{_qt5_docdir}
rm -rf %{buildroot}%{_qt5_importdir}/
rm -rf %{buildroot}%{_qt5_translationdir}/
rm -rf %{buildroot}%{_qt5_archdatadir}/
rm -rf %{buildroot}%{_qt5_datadir}/
rm -rf %{buildroot}%{_qt5_libexecdir}/
rm -rf %{buildroot}%{_qt5_libdir}/libQt5Concurrent.prl %{buildroot}%{_qt5_libdir}/libQt5DBus.prl %{buildroot}%{_qt5_libdir}/libQt5Gui.prl %{buildroot}%{_qt5_libdir}/libQt5OpenGL.prl %{buildroot}%{_qt5_libdir}/libQt5PrintSupport.prl %{buildroot}%{_qt5_libdir}/libQt5Sql.prl %{buildroot}%{_qt5_libdir}/libQt5Test.prl %{buildroot}%{_qt5_libdir}/libQt5XcbQpa.prl %{buildroot}%{_qt5_libdir}/libQt5Xml.prl %{buildroot}%{_qt5_libdir}/libQt5EglFSDeviceIntegration.prl
rm -rf %{buildroot}%{_qt5_libdir}/metatypes/
rm -rf %{buildroot}%{_qt5_libdir}/pkgconfig/Qt5.pc %{buildroot}%{_qt5_libdir}/pkgconfig/Qt5Concurrent.pc
rm -rf %{buildroot}%{_qt5_libdir}/pkgconfig/Qt5DBus.pc %{buildroot}%{_qt5_libdir}/pkgconfig/Qt5Gui.pc
rm -rf %{buildroot}%{_qt5_libdir}/pkgconfig/Qt5OpenGL.pc %{buildroot}%{_qt5_libdir}/pkgconfig/Qt5PrintSupport.pc
rm -rf %{buildroot}%{_qt5_libdir}/pkgconfig/Qt5Sql.pc %{buildroot}%{_qt5_libdir}/pkgconfig/Qt5Test.pc
rm -rf %{buildroot}%{_qt5_libdir}/pkgconfig/Qt5Xml.pc
%if 0%{?egl}
rm -rf %{buildroot}%{_qt5_libdir}/libQt5EglFsKmsSupport.*
%endif
rm -rf %{buildroot}%{_qt5_headerdir}/QtConcurrent/
rm -rf %{buildroot}%{_qt5_headerdir}/QtDBus/
rm -rf %{buildroot}%{_qt5_headerdir}/QtGui/
...
rm -rf %{buildroot}%{_qt5_headerdir}/QtSql/
rm -rf %{buildroot}%{_qt5_headerdir}/QtTest/
rm -rf %{buildroot}%{_qt5_headerdir}/QtXcb/
rm -rf %{buildroot}%{_qt5_headerdir}/QtXml/
rm -rf %{buildroot}%{_qt5_headerdir}/QtEglFSDeviceIntegration
...
rm -rf %{buildroot}%{_qt5_libdir}/qt5/bin/tracegen

rm -rf %{buildroot}%{_bindir}
rm -rf %{buildroot}%{_qt5_bindir}

%if 0%{?examples}
rm -rf %{buildroot}%{_qt5_examplesdir}/
%endif

rm -rf %{buildroot}%{_qt5_libdir}/*.*a

rm -rf %{buildroot}%{_qt5_libdir}/libQt5Bootstrap.prl
rm -rf %{buildroot}%{_qt5_headerdir}/QtOpenGLExtensions/
rm -rf %{buildroot}%{_qt5_libdir}/libQt5OpenGLExtensions.prl
rm -rf %{buildroot}%{_qt5_libdir}/pkgconfig/Qt5OpenGLExtensions.pc
rm -rf %{buildroot}%{_qt5_libdir}/libQt5AccessibilitySupport.prl
rm -rf %{buildroot}%{_qt5_headerdir}/QtAccessibilitySupport
rm -rf %{buildroot}%{_qt5_libdir}/libQt5DeviceDiscoverySupport.prl
rm -rf %{buildroot}%{_qt5_headerdir}/QtDeviceDiscoverySupport
rm -rf %{buildroot}%{_qt5_libdir}/libQt5EglSupport.prl
rm -rf %{buildroot}%{_qt5_headerdir}/QtEglSupport
rm -rf %{buildroot}%{_qt5_libdir}/libQt5EventDispatcherSupport.prl
rm -rf %{buildroot}%{_qt5_headerdir}/QtEventDispatcherSupport
rm -rf %{buildroot}%{_qt5_libdir}/libQt5FbSupport.prl
rm -rf %{buildroot}%{_qt5_headerdir}/QtFbSupport
rm -rf %{buildroot}%{_qt5_libdir}/libQt5FontDatabaseSupport.prl
rm -rf %{buildroot}%{_qt5_headerdir}/QtFontDatabaseSupport
rm -rf %{buildroot}%{_qt5_libdir}/libQt5GlxSupport.prl
rm -rf %{buildroot}%{_qt5_headerdir}/QtGlxSupport
rm -rf %{buildroot}%{_qt5_libdir}/libQt5InputSupport.prl
rm -rf %{buildroot}%{_qt5_libdir}/libQt5LinuxAccessibilitySupport.prl
rm -rf %{buildroot}%{_qt5_headerdir}/QtLinuxAccessibilitySupport
rm -rf %{buildroot}%{_qt5_libdir}/libQt5PlatformCompositorSupport.prl
rm -rf %{buildroot}%{_qt5_headerdir}/QtPlatformCompositorSupport
rm -rf %{buildroot}%{_qt5_libdir}/libQt5ServiceSupport.prl
rm -rf %{buildroot}%{_qt5_headerdir}/QtServiceSupport
rm -rf %{buildroot}%{_qt5_libdir}/libQt5ThemeSupport.prl
rm -rf %{buildroot}%{_qt5_headerdir}/QtThemeSupport
rm -rf %{buildroot}%{_qt5_libdir}/libQt5KmsSupport.prl
rm -rf %{buildroot}%{_qt5_headerdir}/QtKmsSupport
rm -rf %{buildroot}%{_qt5_libdir}/libQt5EdidSupport.prl
rm -rf %{buildroot}%{_qt5_libdir}/libQt5XkbCommonSupport.prl
%if 0%{?vulkan}
rm -rf %{buildroot}%{_qt5_headerdir}/QtVulkanSupport/
rm -rf %{buildroot}%{_qt5_libdir}/libQt5VulkanSupport.prl
%endif
rm -rf %{buildroot}%{rpm_macros_dir}/macros.qt5-qtbase

cp -a include/QtCoreo3de/QtCoreDepends %{buildroot}%{_qt5_headerdir}/QtCoreo3de/
cp -a include/QtNetworko3de/QtNetworkDepends %{buildroot}%{_qt5_headerdir}/QtNetworko3de/
cp -a include/QtWidgetso3de/QtWidgetsDepends %{buildroot}%{_qt5_headerdir}/QtWidgetso3de/

cp -a %{buildroot}%{_qt5_libdir}/cmake/Qt5Core/* %{buildroot}%{_qt5_libdir}/cmake/Qt5Coreo3de/
rm -rf %{buildroot}%{_qt5_libdir}/cmake/Qt5Core

pushd %{buildroot}%{_qt5_headerdir}
cd QtCoreo3de/%{version}/
ln -s QtCoreo3de QtCore
cd ..
ln -s QtCore QtCoreo3de
cd ..
cd QtNetworko3de/%{version}/
ln -s QtNetworko3de QtNetwork
cd ..
ln -s QtNetwork QtNetworko3de
cd ..
cd QtWidgetso3de/%{version}/
ln -s QtWidgetso3de QtWidgets
cd ..
ln -s QtWidgets QtWidgetso3de
cd ..
...
for i in QtCoreo3de QtNetworko3de QtWidgetso3de; do
	for j in `find %{buildroot}/usr/include/qt5/$i/ -type f`; do
		sed -i 's|#include <QtCore/|#include <QtCoreo3de/|g' $j
		sed -i 's|#include <QtNetwork/|#include <QtNetworko3de/|g' $j
		sed -i 's|#include <QtWidgets/|#include <QtWidgetso3de/|g' $j
	done
done
...
%{_qt5_libdir}/libQt5Coreo3de.so.5*
%{_qt5_libdir}/libQt5Networko3de.so.5*

%files devel
%if "%{_qt5_headerdir}" != "%{_includedir}"
%dir %{_qt5_headerdir}
%endif
%{_qt5_headerdir}/QtCoreo3de/
%{_qt5_headerdir}/QtNetworko3de/
%{_qt5_headerdir}/QtWidgetso3de/
%{_qt5_libdir}/cmake/Qt5Coreo3de/
%{_qt5_libdir}/cmake/Qt5Networko3de/
%{_qt5_libdir}/cmake/Qt5Widgetso3de/
%{_qt5_libdir}/libQt5Coreo3de.prl
%{_qt5_libdir}/libQt5Coreo3de.so
%{_qt5_libdir}/libQt5Networko3de.prl
%{_qt5_libdir}/libQt5Networko3de.so
%{_qt5_libdir}/libQt5Widgetso3de.prl
%{_qt5_libdir}/libQt5Widgetso3de.so
%{_qt5_libdir}/pkgconfig/Qt5Coreo3de.pc
%{_qt5_libdir}/pkgconfig/Qt5Networko3de.pc
%{_qt5_libdir}/pkgconfig/Qt5Widgetso3de.pc
...
%files gui
%{_qt5_libdir}/libQt5Widgetso3de.so.5*

%changelog
* Tue Apr  6 2021 Tom Callaway <spot@fedoraproject.org>
- create an o3de fork of the Fedora package that can co-exist with the Fedora qt5 packages
